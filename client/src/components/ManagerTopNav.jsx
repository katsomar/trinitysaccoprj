...
export default function ManagerTopNav() {
  ...
  function handleSearch(e) {
    ...
    if (searchType === "group") {
      setResults(mockGroups.filter(g => g.name.toLowerCase().includes(val.toLowerCase())));
    } else {
      setResults(mockUsers.filter(u => u.name.toLowerCase().includes(val.toLowerCase()) || u.username.toLowerCase().includes(val.toLowerCase())));
    }
  }

  function handleMobileSearch(e) {
    const val = e.target.value; setMobileSearch(val);
    ...
    if (mobileSearchType === "group") {
      ...
      setMobileResults(mockUsers.filter(u => u.name.toLowerCase().includes(val.toLowerCase()) || u.username.toLowerCase().includes(val.toLowerCase())));
    }
  }
  ...
  // Avoid duplicate hamburger/menu on the manager dashboard page
  ...
  return (
    <>
                      ...
                      {results.length > 0 ? results.map(u => (
                        <div className="discover-trending-row" key={u.id}>
                          ...
                          <div className="discover-user-actions">
                            <button className="discover-chat-btn" onClick={() => navigate('/manager-chat')}>Chat</button>
                            <button className="discover-invite-btn" onClick={() => alert('Invite flow (mock)')}>Invite</button>
                          </div>
                        </div>
      ...
      {/* Hamburger + mobile menu for all manager pages except manager dashboard */}
      {!isManagerDashboard && (
        <>
          <button
            ...
            aria-controls="manager-mobile-side-menu"
          ...
          <nav id="manager-mobile-side-menu" className={`mobile-side-menu ${mobileMenuOpen ? 'open' : ''}`} role="dialog" aria-modal="false" aria-label="Mobile Navigation">
            ...
            <div className="mobile-menu-section mobile-search">
              <label className="mobile-label" htmlFor="manager-mobile-search">Search</label>
      ...
      {/* Mobile styles for manager pages */}
      <style>{`
        :root { --nav-h: 56px; }
        /* Left align profile on small devices */
        ...
        /* Hide desktop search/logout on tablet/phone; use side menu instead */
        @media (max-width: 1024px) {
          .navbar .navbar-center { display: none; }
          .navbar .logout-btn { display: none !important; }
        }
        /* Hamburger toggle */
        .hamburger-toggle { position: fixed; top: 10px; right: 14px; width: 42px; height: 42px; border-radius: 10px; border: none; background: rgba(255,255,255,0.12); display: none; align-items: center; justify-content: center; z-index: 1001; cursor: pointer; transition: background 0.2s ease; }
        ...
        /* Mobile side menu */
        .mobile-side-menu { position: fixed; top: var(--nav-h); left: 0; width: min(84vw, 340px); height: calc(100vh - var(--nav-h)); background: rgba(255,255,255,0.72); backdrop-filter: blur(6px); -webkit-backdrop-filter: blur(6px); border-right: 1px solid rgba(0,0,0,0.06); box-shadow: 0 10px 30px rgba(0,0,0,0.08); transform: translateX(-102%); transition: transform 0.32s ease; z-index: 1000; padding: 1rem 1rem 1.2rem 1rem; overflow-y: auto; }
